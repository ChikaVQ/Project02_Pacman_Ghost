...
Pacman uses momentum-based BFS to intercept ghost.
Ghost tries to avoid pacman while exploring.
...
class BeliefTracker:
    ...
    def update(self, obs_map: np.ndarray, memory_map: np.ndarray, enemy_pos):
        ...
        # reduce belief in visible areas where we don't see the enemy
        visible = (obs_map != -1)
        self.belief[visible] *= self.visible_decay
        self.belief[memory_map == 1] = 0.0

        # normalize to keep beliefs as probabilities
    ...
    def _propagate(self, possible_mask: np.ndarray):
        ...
        # normalize so belief doesn't drift to zero
        s = float(newb.sum())
        if s <= 1e-9:
            self.belief = newb
        else:
    ...
    def best_guess(self, last_known=None):
        ...
        if last_known is not None:
            # prioritize cells close to last known position
            cands.sort(key=lambda p: manhattan(p, last_known))
            cands = cands[:8]

        # pick random to avoid getting stuck in loops
...
class PacmanAgent(BasePacmanAgent):
    def __init__(self, **kwargs):
        ...
        # need to be within this distance to capture
    ...
    def _frontiers(self):
        # find known cells next to unexplored areas
    ...
    def _allowed_steps(self, mv: Move, last_mv: Move):
        if mv == Move.STAY:
            return [1]
        if mv != last_mv:
            return [1]  # turning = 1 step only
        if self.pacman_speed >= 2:
            return [2, 1]  # straight = can do 2 or 1
        return [1]
    ...
    def _capture_cells(self, ghost_pos):
        # cells within capture range of ghost
    ...
    # BFS pathfinding with movement rules
    def _estimate_turns_single(self, start, last_mv, goal, max_turns=18):
        # count turns needed to reach goal
        if start == goal:
            return 0
    ...
    def _best_first_action_to_targets(
        ...
        def tie_value(curr_pos, curr_last_mv, mv, st_requested, moved_pos, moved_steps):
            # score moves based on momentum, backtracking, and visit history
            val = 0.0

            # avoid going back to previous cell
            ...
            # bonus for 2-step moves when not too close to target
            if moved_steps == 2:
        ...
        while q:
            ...
            for mv in ALL_MOVES:
                for st in self._allowed_steps(mv, last_mv):
                    ...
                    # track first action to reach this state
            ...
            # optionally explore through unknown cells when close
            if allow_one_unknown_finish and strict_known:
                for mv in ALL_MOVES:
                    # try 1-step through unknown (risky but useful)
    ...
    # predict ghost movement along one axis
    def _axis_escape_predictions(self, ghost_pos, my_pos, steps_ahead=(2, 3, 4)):
        # ghost moves along row or column, not diagonally
        if ghost_pos is None:
            return []
    ...
    def step(self, map_state: np.ndarray, my_position: tuple, enemy_position: tuple, step_number: int):
        ...
        # already close enough to capture
        ...
        if ghost_est is not None:
            ...
            allow_unknown = (dist_to_est <= 4)  # explore through fog if close
            ...
            if act:
                ...
                # only move through known walkable cells
        ...
        if frontiers:
            ...
            sample = frontiers[:30]  # keep it fast
        ...
        # fallback: just move somewhere useful
        prev_cell = self.prev_positions[-2] if len(self.prev_positions) >= 2 else None
        candidates = []
        for mv in ALL_MOVES:
            for st in self._allowed_steps(mv, self.last_move):
                ...
                if moved == 2 and ghost_est is not None and manhattan(my_position, ghost_est) <= 3:
                    score -= 30.0  # slow down when close
...
# =========================================================
# Ghost agent - tries to hide from pacman
# =========================================================
